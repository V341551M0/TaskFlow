...
**Decisão:** um único script sobe a infraestrutura local de forma idempotente, detectando o MySQL a usar: primeiro tenta o **MySQL do sistema** (`127.0.0.1:3306`, persistente) e, sem acesso, cai para uma **instância isolada** em `/tmp/taskflow-mysql` na porta `3307`. Quando o banco do sistema está disponível, o script **encerra e remove** a instância isolada (redundante) e reinicia a API apontando para o banco persistente.
